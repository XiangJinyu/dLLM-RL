"""
Agent RL training script for LLaDA-based diffusion language models.

Extends rl_llada.py to support multi-turn agent trajectories:

Key differences from standard rl_llada.py:
1. **Segment-aware masking**: Only model-generated tokens are trained on.
   Observation/environment tokens are frozen (not masked during training).
2. **Multi-turn step_map**: The step_map spans the full trajectory but
   respects segment boundaries -- each turn's diffusion steps are kept
   separate and correctly offset.
3. **Turn-level reward support**: Optional per-turn rewards instead of
   a single scalar per trajectory. Supports both trajectory-level and
   turn-level credit assignment.
4. **Extended prompting**: The prompt now includes system messages,
   tool descriptions, and interleaved observations.

The training objective is the same PPO-clipped loss from TraceRL,
but the masking strategy is segment-aware.
"""
...
class AgentTrainDataset(Dataset):
    """
    Dataset for agent RL training.

    Extends TrainDataset with segment awareness:
    - trainable_mask: boolean mask indicating which tokens are model-generated
      (should be trained on) vs. environment-injected (should be frozen)
    """

    def __init__(self, inputs, labels, pmasks, reward, trainable_masks=None):
        self.inputs = inputs
        self.labels = labels
        self.pmasks = pmasks
        self.reward = reward
        self.trainable_masks = trainable_masks  # (N, L) bool tensor
        L_raw = inputs.shape[1]
        self.logp_old_tok = torch.full((len(inputs), L_raw), float("-inf"))
...
def main():
    ...
    def build_trainable_mask(
        token_segments: List[Tuple[int, int, bool]], total_len: int, prompt_len: int
    ) -> torch.Tensor:
        """
        Build a boolean mask indicating which tokens are trainable (model-generated).

        token_segments: list of (start_tok, end_tok, is_trainable)
        total_len: total sequence length
        prompt_len: length of the initial prompt (not trainable)

        Returns: (total_len,) boolean tensor
        """
        mask = torch.zeros(total_len, dtype=torch.bool)
        for seg_start, seg_end, is_trainable in token_segments:
            if is_trainable:
                # Clamp to valid range
                s = max(seg_start, prompt_len)
                e = min(seg_end, total_len)
                if s < e:
                    mask[s:e] = True
        return mask
    ...
    @torch.no_grad()
    def prepare_agent_inputs_and_labels(
        prompt,
        response,
        step_map,
        reward,
        ...
        """
        Prepare inputs for agent RL training with segment-aware masking.

        This is the key function that differentiates agent training from
        standard single-turn training. It:

        1. Tokenizes the full trajectory (prompt + multi-turn response)
        2. Builds a trainable_mask that excludes observation tokens
        3. Applies TraceRL/random masking only to trainable regions
        4. Uses the step_map that respects multi-turn boundaries
        """
        ...
        # Build trainable masks for each sample
        trainable_masks = []
        for b in range(B):
            if b < len(token_segments_list) and token_segments_list[b]:
                tmask = build_trainable_mask(token_segments_list[b], L_after, start_pos)
            else:
                # Fallback: treat everything after start_pos as trainable
                tmask = torch.zeros(L_after, dtype=torch.bool)
                tmask[start_pos:] = True
            trainable_masks.append(tmask)
        trainable_masks_tensor = torch.stack(trainable_masks)
        ...
        if config.training.method == "TraceRL":
            ...
            for b in range(B):
                ...
                # Only assign step_map values to trainable positions
                resp_len = min(len(order_list), L_after - start_pos)
                order_full[start_pos : start_pos + resp_len] = order[:resp_len]

                # Get trainable mask for this sample
                tmask_b = trainable_masks_tensor[b].to(device)

                uniq_steps = torch.unique(order_full[start_pos:], sorted=True)
                # Remove -1 (non-response positions)
                ...
                if config.training.post_num is not None:
                    ...
                    keep_first_pad_b = pad_mask_b & (
                        torch.cumsum(pad_mask_b.int(), dim=0)
                        <= config.training.post_num
                    )
                    tail_pad_b = pad_mask_b & ~keep_first_pad_b
                else:
                    keep_first_pad_b = torch.zeros(
                        L_after, dtype=torch.bool, device=device
                    )
                    tail_pad_b = torch.zeros(L_after, dtype=torch.bool, device=device)

                for step_val in uniq_steps:
                    tgt_mask = order_full == step_val
                    # CRITICAL: Only mask trainable positions
                    pmask_this = tgt_mask & ~tail_pad_b & tmask_b
                    ...
                    # Also mask non-trainable observation tokens
                    # (they are already in base_ids, so no change needed)
            ...
            if not noisy_list:
                # No valid training data -- return empty tensors
                return (
                    torch.zeros(0, L_after, dtype=torch.long),
                    torch.zeros(0, L_after, dtype=torch.long),
                    torch.zeros(0, L_after, dtype=torch.bool),
                    [],
                    start_pos,
                    drop_num,
                )
            ...
            for b in range(B):
                ...
                for _ in range(m):
                    ...
                    # CRITICAL: Only mask trainable positions
                    rand_mask = rand_mask & tmask_b & ~tail_pad_b
                    ...
                    noisy_ids[tail_pad_b & tmask_b] = mask_id
            ...
            for b in range(B):
                ...
                for _ in range(m):
                    ...
                    if rand_mask.any():
                        ...
                        noisy_rand[tail_pad_b & tmask_b] = mask_id
                    ...
                    if comp_mask.any():
                        ...
                        noisy_comp[tail_pad_b & tmask_b] = mask_id
        ...
        # Filter rows where p_mask is all False
    ...
    def compute_logp_old_tok_parallel(
        accelerator, dataset, train_dataloader_lm, start_pos, pad_id, batch_size
    ):
        model.eval()
        for batch in train_dataloader_lm:
            ...
            tok_lp = log_probs.gather(dim=-1, index=safe_labels.unsqueeze(-1)).squeeze(
                -1
            )
    ...
    #       Preprocess data          #
    #################################
    logger.info("Preprocessing agent training data")
    ...
    for x in dataset_load:
        ...
        # Agent-specific: token segments for trainable mask
        token_segments_list.append(x.get("token_segments", []))

    if not prompt_list:
        logger.warning("No training data! Exiting.")
        return

    input_ids, labels, p_mask_lm, rewards, start_pos, drop_num = (
        prepare_agent_inputs_and_labels(
            prompt_list,
            response_list,
            step_map_list,
            reward_list,
    ...
    if input_ids.shape[0] == 0:
        logger.warning("No valid training samples after masking! Exiting.")
        return

    dataset_lm = AgentTrainDataset(input_ids, labels, p_mask_lm, rewards)
    ...
    #             Inference         #
    #################################
    logger.info("***** Running inference for old log-probs *****")
    ...
    #             Training          #
    #################################
    logger.info("***** Running agent RL training *****")
    ...
    logger.info(
        f"  Instantaneous batch size per device = {config.training.batch_size_lm}"
    )
    ...
    def forward_process(input_ids, labels, p_mask_lm, adv, logp_old_tok):
        """PPO-clipped loss with optional KL penalty -- same as standard rl_llada.py."""
        logits = model(input_ids).logits
        B, T, V = logits.shape
    ...
    if config.experiment.current_epoch % config.experiment.save_every == 0:
        save_checkpoint(
            ...
            config,
            accelerator,
            f"epoch-{config.experiment.current_epoch}",
        )
...
def save_checkpoint(model, tokenizer, config, accelerator, name):
    """Save model checkpoint -- identical to rl_llada.py."""
